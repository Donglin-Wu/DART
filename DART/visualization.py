...
def Save1DVisualizationSvg(outputPath, xPlot, meanFusion, stdFusion, xLow, yLow, xHigh, yHigh, bhf, iterations, xVal=None, yVal=None, xLabel="Input variable", yLabel="Output variable"):
    width = 1200
    height = 800
    left = 110
    right = 40
...
<text x="{width / 2}" y="35" text-anchor="middle" font-family="Arial" font-size="24">Dual-fidelity Fusion Result</text>
<text x="{width / 2}" y="62" text-anchor="middle" font-family="Arial" font-size="16">BHF = {bhf}    Iterations = {iterations}</text>
...
<text x="{width - 252}" y="104" font-family="Arial" font-size="14">95% confidence interval</text>
<line x1="{width - 280}" y1="132" x2="{width - 258}" y2="132" stroke="#2f7ed8" stroke-width="2.5" />
<text x="{width - 252}" y="136" font-family="Arial" font-size="14">Fusion model</text>
<circle cx="{width - 271}" cy="160" r="4" fill="#4f81bd" opacity="0.55" />
<text x="{width - 252}" y="165" font-family="Arial" font-size="14">Low-fidelity samples</text>
<circle cx="{width - 271}" cy="188" r="5" fill="#c0504d" />
<text x="{width - 252}" y="193" font-family="Arial" font-size="14">High-fidelity samples</text>
<line x1="{width - 280}" y1="216" x2="{width - 258}" y2="216" stroke="black" stroke-width="2" />
<text x="{width - 252}" y="220" font-family="Arial" font-size="14">True high-fidelity curve</text>
</svg>"""
...
def Save2DVisualizationSvg(outputPath, xLow, yLow, xHigh, yHigh, xGrid, yGrid, zGrid, bhf, iterations, xVal=None, yVal=None, inputLabels=None, outputLabel="Output variable"):
    width = 1400
    height = 1000
    left = 90
    right = 260
    ...
    xLabel = inputLabels[0] if inputLabels and len(inputLabels) > 0 else "Input variable 1"
    yLabel = inputLabels[1] if inputLabels and len(inputLabels) > 1 else "Input variable 2"
    zLabel = outputLabel or "Output variable"

    svgContent = f"""<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">
<rect width="100%" height="100%" fill="white" />
<text x="{width / 2}" y="36" text-anchor="middle" font-family="Microsoft YaHei UI, Arial" font-size="26">Dual-fidelity 3D Fusion Result</text>
<text x="{width / 2}" y="66" text-anchor="middle" font-family="Microsoft YaHei UI, Arial" font-size="16">BHF = {bhf}    Iterations = {iterations}</text>
...
<text x="{width - 190}" y="125" font-family="Microsoft YaHei UI, Arial" font-size="15">Fusion surface</text>
<circle cx="{width - 210}" cy="157" r="4" fill="#4f81bd" opacity="0.5" />
<text x="{width - 190}" y="162" font-family="Microsoft YaHei UI, Arial" font-size="15">Low-fidelity samples</text>
<circle cx="{width - 210}" cy="189" r="4" fill="#c0504d" stroke="#000000" stroke-width="0.8" />
<text x="{width - 190}" y="194" font-family="Microsoft YaHei UI, Arial" font-size="15">High-fidelity samples</text>
<circle cx="{width - 210}" cy="221" r="4" fill="#808080" opacity="0.35" />
<text x="{width - 190}" y="226" font-family="Microsoft YaHei UI, Arial" font-size="15">Validation data</text>
</svg>"""
...
def VisualizeResults(xLow, yLow, xHigh, yHigh, fusionModel, scalerXLow, scalerYLow, bounds, dim, iterations, bhf, outputDir, xVal=None, yVal=None, inputLabels=None, outputLabel="Output variable"):
    """
    Export 1D or 2D fusion result plots.
    """
    ...
    if dim == 1:
        ...
        xLabel = inputLabels[0] if inputLabels else "Input variable"
        yLabel = outputLabel or "Output variable"

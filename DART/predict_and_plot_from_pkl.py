"""
Generate prediction results and regression-fit scatter plots from a trained pkl model.
"""
...
<text x="{width / 2}" y="35" text-anchor="middle" font-family="Arial" font-size="24">Regression Fit Scatter Plot</text>
...
def GeneratePredictionAndScatter(modelPath, inputCsvPath, outputCsvPath, truthCsvPath=None, scatterSvgPath=None):
    ...
    SaveScatterSvg(
        ...
        maeValue=maeValue,
        r2Value=r2Value,
        xLabel="True physical value",
        yLabel="Fusion model prediction",
    )
...
def Main():
    parser = argparse.ArgumentParser(description="Generate prediction results and a regression-fit scatter plot from a trained pkl model.")
    parser.add_argument("--model", required=True, help="Path to the trained trained_model.pkl file")
    parser.add_argument("--input", required=True, help="Input CSV path used to generate predictions")
    parser.add_argument("--truth", help="True high-fidelity CSV path used for the regression-fit scatter plot")
    parser.add_argument("--output-dir", help="Output directory; if omitted, a timestamped FusionResult directory is created automatically")
    parser.add_argument("--output-csv", help="Prediction result CSV path")
    parser.add_argument("--scatter-svg", help="Regression-fit scatter plot SVG path")
    ...
    print(f"Prediction results saved: {resultInfo['predictionCsv']}")
    print(f"Regression-fit scatter plot saved: {resultInfo['scatterSvg']}")

"""
Lightweight NumPy-based Kriging surrogate model.
"""
...
def SafeCholesky(matrixData, baseJitter=1e-10, maxRetries=8):
    matrixData = np.asarray(matrixData, dtype=float).copy()
    dimension = matrixData.shape[0]
    jitterValue = float(max(baseJitter, 1e-12))

    # Medium-scale nearly singular matrices can terminate the underlying process in some Windows + NumPy linear algebra environments.
    # Small matrices still use np.linalg.cholesky, while large matrices use a more stable row-wise factorization.
    if dimension <= 96:
        for _ in range(maxRetries):
            try:
                return np.linalg.cholesky(matrixData)
            except np.linalg.LinAlgError:
                matrixData[np.diag_indices_from(matrixData)] += jitterValue
                jitterValue *= 10.0

    for _ in range(maxRetries):
        lowerMatrix = np.zeros_like(matrixData, dtype=float)
        success = True

        for rowIndex in range(dimension):
            diagonalTerm = matrixData[rowIndex, rowIndex] - np.dot(lowerMatrix[rowIndex, :rowIndex], lowerMatrix[rowIndex, :rowIndex])
            if diagonalTerm <= 1e-12:
                success = False
                break
            lowerMatrix[rowIndex, rowIndex] = np.sqrt(diagonalTerm)

            for colIndex in range(rowIndex + 1, dimension):
                numerator = matrixData[colIndex, rowIndex] - np.dot(lowerMatrix[colIndex, :rowIndex], lowerMatrix[rowIndex, :rowIndex])
                lowerMatrix[colIndex, rowIndex] = numerator / lowerMatrix[rowIndex, rowIndex]
    ...
    raise np.linalg.LinAlgError("Covariance matrix Cholesky decomposition failed.")
...
class SimpleKrigingModel:
    ...
    def Train(self):
        if self.xTrain is None or self.yTrain is None:
            raise ValueError("Training data must be set before fitting.")
        ...
        self.choleskyFactor = SafeCholesky(covarianceMatrix, baseJitter=max(self.nuggetVal, 1e-10))
